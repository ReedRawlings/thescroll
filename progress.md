...
## Apply reviewed placement CSV — 2026-09-17
- Read the user's revised Downloads CSV; preserved it exactly as docs/design/asset-placement.csv with its six columns. Added runtime catalog asset-rules.js and original individual sprite copies. All generated art stays in the supplied orientation, without rotation or flipping.
- Reworked interior recipes into four distinct purposes per floor: reading room, supply store, burial chamber, audience chamber. Added shelf/table/chair relationships, display boards, tabletop items, supported debris, rare throne grouping, solid standing lamp bases and sparse wall paintings. Footprints use source dimensions while distinguishing sprite height from ground depth.
- Chests now block movement as requested in the CSV. Tapping finds an adjacent approach, awards contents once, and uses the supplied six-frame opening strip. Routes/validation target the approach. Legacy authored floor chest behavior remains supported.
- 28 model tests pass, including 200 geometry seeds, 80 reviewed-placement seeds, associations/orientation, solid chest interaction and reward-once behavior. Build passes. Three desktop/mobile browser playthroughs pass with no errors, including loot/combat/stairs/reset/reroll. Inspected final overview and mobile captures under artifacts/bsp.
- Reviewed CSV notes about future switches/locks/hazards remain deferred rather than silently adding mechanics. The catalog is a reviewed snapshot; later CSV edits need an explicit corresponding update.

- Replaced per-tile random BSP flooring with dominant room fields, clustered wear, corner details, and connected doorway paving. Preserved all 16 source variants across seeds and native orientation.
- Fixed stalled furniture recipes; added area-scaled themed groups with edge preference and interior fallback, preserving doorway clearance and connectivity.
- Verified live screenshot for stone-42x4sl; 29 unit tests and production build pass. Existing 5173 server was serving another build; current Vite preview started on 5174. Browser journey test now accepts BSP_BASE_URL.
- Browser journeys passed for all three seeds (including mobile): treasure, combat, movement, supplies, stairs, reset/reroll; no browser errors.

- Added optional third-level BSP subdivision of larger partitions, capped at six rooms, with minimum 5×5 interiors. Updated room descriptions and theme assignment; retained three enemies per floor.
- Reserved treasure before furnishing; scaled floor wear down in small chambers. 1,500 seeds generated successfully: 400 four-room, 638 five-room, 462 six-room layouts. 29 unit tests and production build pass. Visual inspection uses seed 8 (six rooms); browser journeys use seeds 1, 0, 8 for four/five/six coverage.
- Four-, five-, and six-room browser journeys all completed successfully with no console errors, including mobile movement, combat, treasure, and stairs. Inspected six-room screenshot and opened seed 8 for review.

- Added /?editor=1 Room workshop: floor painting with automatic walls, all 69 reviewed assets, tabletop support, markers, metadata, three starter rooms, Undo, persistent browser library/draft, validated JSON import/export, and click/keyboard walk test.
- Room checks detect disconnected floor, blocked markers, and invalid entrance locations. Templates must pass checks to save; reference drafts may retain warnings. Generator insertion remains a future integration and is explicitly labeled in UI and docs.
- 32 unit tests and build pass. Browser checks cover editing, collision, walk routes, undo, save/reload, export/import and malformed files; desktop/mobile screenshots inspected. Added workshop link to floor preview.
- Added editor Undo/Redo shortcuts (Cmd/Ctrl Z, Cmd/Ctrl Shift Z, Ctrl Y), redo button, and native text-field shortcut preservation. Added selected-asset preview and an explicit Replace asset tool with placement validation and rollback. Keyboard and replacement coverage added to editor browser tests.
- Expanded editor catalog with original tile-sheet bridges, acid pools, pillars, carpets, and stairs. Added explicit Void / gap terrain, suppressed wall generation there, layered carpets/acid beneath props, and bridge traversal over acid/void. Existing reviewed CSV/catalog unchanged outside editor.
- 33 unit tests and build pass; editor browser regression and explicit bridge-crossing check passed. Inspected expanded study screenshot and corrected a bridge crop containing two disconnected source pieces. Acid is blocked terrain; stairs are artwork only in walk test.
- Fixed doorway authoring: renamed Entrance button to Doorway, accepted wall-adjacent placement (including north cap selection), removed wall artwork at openings, and made doorway cells walkable. Existing floor-edge markers remain compatible. Removing a doorway restores the wall. All four sides covered by tests; browser walk test reaches a new north-wall opening. 34 tests and build pass.
- Split each combined stair sprite down its source midpoint into separate unrotated left/right halves (eight pieces). Import migrates old combined placements into matching halves.
- Added Raised edge paint tool and left/middle/right front-face assets; faces block walking, stairs stay walkable. Assembled and visually inspected a raised platform matching the user's reference composition. 35 tests, build and editor browser checks pass.
- Added six rounded wall stamps (upper/lower left, middle, right), eight smaller/shallow native stair halves, and three straight stair stamps from the supplied sheets. Rounded walls use solid footprints; stairs stay walkable, all unrotated. Visually inspected palette and assembled room. 35 tests, build, and editor browser checks pass.
- Added rounded bottom-only wall crops and grouped straight/rounded faces under Wall bottoms. These can be placed on void and suppress generated wall tops beneath their placement. Renamed tools to Wall bottom / Paint void, added void swatch to floor palette, and preserved bridges/ledges when painting void under them.
- Verified void painting on existing floor, retained wall faces, Undo and export/import in browser. 36 unit tests and build pass.
- Door assets now mount on walls and derive full-width connection openings without separate markers. North cap clicks snap to face; removal restores walls; exports preserve connections through the placed asset. Door preview is traversable, including closed artwork, with locks/opening behavior explicitly not simulated. Added unit and browser traversal coverage.
- Fixed destructive terrain edits: Paint void and Erase terrain now modify only terrain, preserving every placed asset and marker. Removed implicit asset cleanup during saving. Import restores independent asset placements even where underlying terrain was removed; stairs, like bridges, supply walkable surfaces over void. Regression covers persistence and traversal.
- Integrated BSP generator into main game behind /?generated=1 and town link. One-floor expedition uses normal movement/pursuit/combat/rewards/inventory/extraction; stairs finish and bank winnings. Reviewed terrain/props render at native pixel scale. Solid chests can be tapped to approach and looted orthogonally.
- Added seed input, visible seed, same-seed pause restart, separate generated save/meta namespaces. Fixed arbitrary-text seed combat RNG while preserving standard-run RNG behavior.
- 38 unit tests and build pass. Full seed 8 desktop and seed 1 mobile generated journeys pass (three battles each, chest, 69 gold, stairs, restart/resume); standard three-floor journey passes with ten battles. Inspected mobile town/exploration and pixel-art screenshot. Authored template insertion remains next milestone.
- Added Try another seed on generated town, pause, and completion screens. Picker starts a custom seed or random floor; URL updates to current seed. Same-seed restart retained and picker Back restores prior modal. Mobile browser checks cover custom/random seed and reproducible restart; build passes.
- Responded to seed 14001 feedback with actual geometry cuts (clipped/recessed/L-shaped), floor-area-aware furnishings, direct/offset/unmarked paths and encounter counts scaling with rooms/area instead of fixed three.
- Seed 14001 visually inspected: six irregular rooms, five encounters. 1,500 seeds generated without connectivity failures. Main-game seed 14001 playthrough reached completion without browser errors (three encounters fought en route; remaining encounters optional). Added regression for shape variety, encounter count, route modes and no flooring in cutout cells.
- Increased room contour variety with occasional deeper recesses (roughly 45–65% width, 50–70% depth), stronger clipped corners, and occasional opposite notches in large rooms. Minimum remaining arms are three tiles; corridor and connectivity protections retained. 2,000 seed audit, 39 tests, and production build pass; inspected updated seed 14001.
- Promoted BSP generation/art to the default three-floor expedition. Added generateExpeditionFloor: floor one matches the seed preview, later floors use floor-specific seeds; floor three promotes a valid stair-room encounter to the Warden. Reviewed sprites preload in standard runs; legacy stored floor maps continue rendering unchanged.
- 40 tests and build pass, including 100 seeds × three floors and reachable boss validation. Standard main-game browser journey completed 12 battles and final victory with no errors. Verified old-generator saved floor resumes with its map and gold intact. One-floor generation testing remains available.
