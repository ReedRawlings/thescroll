...
await mkdir("artifacts/bsp", { recursive: true });
    ...
    if (state.battle.phase === "command") {
      if (!pauseChecked) {
        ...
        await page.screenshot({
          path: "artifacts/bsp/combat.png",
          fullPage: true,
        });
        await page.setViewportSize({ width: 390, height: 844 });
        await page.screenshot({
          path: "artifacts/bsp/mobile-combat.png",
          fullPage: true,
        });
        assert.ok(
          await page.evaluate(
...
try {
  ...
  for (let i = 0; i < 3; i++) {
    ...
    let state = await read();
    await page.screenshot({
      path: `artifacts/bsp/${state.seed}.png`,
      fullPage: true,
    });
    await page
      .locator("canvas")
      .screenshot({ path: `artifacts/bsp/${state.seed}-map.png` });
    ...
    if (i === 0)
      await page.screenshot({
        path: "artifacts/bsp/structure.png",
        fullPage: true,
      });
    await click("decor");
    await click("routes");
    ...
    if (i === 2) {
      await page.setViewportSize({ width: 390, height: 844 });
      await page.screenshot({
        path: "artifacts/bsp/mobile-overview.png",
        fullPage: true,
      });
      assert.ok(
        await page.evaluate(
    ...
    if (i === 2)
      await page.screenshot({
        path: "artifacts/bsp/mobile-play.png",
        fullPage: true,
      });
  ...
  await click("next");
  assert.equal((await read()).floor, "bsp");
  const randomSeed = (await read()).seed;
  const originalMap = (await read()).map;
  await click("reset");
  assert.equal((await read()).seed, randomSeed);
  assert.deepEqual((await read()).map, originalMap);
  await click("reroll");
  assert.notEqual((await read()).seed, randomSeed);
  await click("reset");
  assert.equal((await read()).mode, "explore");
  console.log(JSON.stringify({ passed: true, floors: 3, errors }));
} catch (e) {
  await page.screenshot({
    path: "artifacts/bsp/failure.png",
    fullPage: true,
  });
